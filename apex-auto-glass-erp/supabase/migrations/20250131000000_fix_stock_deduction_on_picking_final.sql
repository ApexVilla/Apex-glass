-- ============================================
-- CORREÇÃO FINAL: Baixa de Estoque na Separação
-- ============================================
-- Esta migração corrige a função create_inventory_movement
-- para que o tipo 'saida_separacao' baixe o estoque automaticamente
-- ao finalizar a separação, conforme solicitado pelo usuário.
-- ============================================
...
BEGIN
    ...
    -- Validar estoque antes de criar movimentação de saída
    ...
    -- CORREÇÃO: saida_separacao AGORA baixa estoque automaticamente ao finalizar separação
    ...
    ELSIF p_type IN ('out', 'saida_venda', 'saida_manual', 'saida_separacao', 'saida_ajuste', 'saida_devolucao_cliente') THEN
        -- CORREÇÃO: Todos os tipos de saída (incluindo saida_separacao) agora baixam estoque
        v_balance_after := v_balance_before - p_quantity;
...
-- 2. Atualizar função calculate_stock_balance_before para incluir saida_separacao no cálculo
...
BEGIN
    ...
    AND type != 'transferencia'; -- Excluir apenas transferencia do cálculo
...
BEGIN
    ...
    ELSIF NEW.type IN ('out', 'saida_venda', 'saida_manual', 'saida_separacao', 'saida_ajuste', 'saida_devolucao_cliente') THEN
        -- CORREÇÃO: Todos os tipos de saída (incluindo saida_separacao) agora alteram o saldo
        v_balance_after := v_balance_before - NEW.quantity;
    ...
    -- Set estoque_anterior e estoque_posterior para compatibilidade
    IF NEW.estoque_anterior IS NULL THEN
        NEW.estoque_anterior := v_balance_before;
    END IF;
    IF NEW.estoque_posterior IS NULL THEN
        NEW.estoque_posterior := v_balance_after;
    END IF;
...
-- 4. Criar trigger para atualizar estoque automaticamente quando inserir movimentação diretamente
-- (caso alguém insira diretamente na tabela sem usar a função)
CREATE OR REPLACE FUNCTION public.update_stock_on_movement_insert()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    -- CORREÇÃO: saida_separacao AGORA baixa estoque
    IF NEW.type IN ('in', 'entrada_compra', 'entrada_manual', 'entrada_ajuste', 
                    'entrada_devolucao_fornecedor', 'entrada_devolucao_cliente') THEN
        UPDATE public.products
...
FOR EACH ROW
EXECUTE FUNCTION public.update_stock_on_movement_insert();

-- Comentário na função
COMMENT ON FUNCTION public.create_inventory_movement IS 'Cria uma movimentação de estoque. CORRIGIDO: saida_separacao agora baixa estoque automaticamente ao finalizar separação. Aceita todos os tipos: entrada_compra, entrada_manual, entrada_ajuste, entrada_devolucao_fornecedor, entrada_devolucao_cliente, saida_venda, saida_manual, saida_separacao, saida_ajuste, saida_devolucao_cliente, transferencia';
